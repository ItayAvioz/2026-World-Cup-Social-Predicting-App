...
import { runFactGate } from './factgate.ts'
...
serve(async (req) => {
  ...
  for (let i = 0; i < qualifyingGroups.length; i++) {
    ...
    try {
      ...
      // 8f-gate: deterministic fact-gate -> gpt-5-mini full regen -> surgical fix (skip test/single-agent mode)
      let finalContent = winner.content
      let gateMeta: Record<string, unknown> = { checked: false }
      if (!testMode && agentPrompts.length > 1) {
        try {
          const res = await runFactGate(openai, payload, winner, winnerPromptRow)
          finalContent = res.content
          gateMeta = res.meta as Record<string, unknown>
          console.log(`[gate] ${group.name}: gate0=${gateMeta.gate0} escalated=${gateMeta.escalated} surgical=${gateMeta.surgical} final=${gateMeta.final_gate}`)
        } catch (e) {
          console.error(`[gate] ${group.name} error:`, (e as Error)?.message)
        }
      }
