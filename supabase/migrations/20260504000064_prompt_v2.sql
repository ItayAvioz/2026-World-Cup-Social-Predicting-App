-- M64: Prompt v2 fixes + v10B as candidate_4 (5th agent)
-- · Add 'candidate_4' to agent_slot CHECK on prompt_versions
-- · Update winner_agent CHECK on ai_judge_runs and ai_summaries (1→5)
-- · INSERT v10B (v10 base + v11-era field names + bad-guessers ban + P4 direction fix + P5 scoreline note)
-- · INSERT v11-main-2 (v11 + P4 global_upset direction rule)
-- · INSERT v12-picks-2 (v12 + P4 direction fix + P5 scoreline note)
-- · INSERT v13-unique-2 (v13 + P4 direction fix + P5 scoreline note + P6 template ban)
-- · Update fn_daily_admin_digest to include v10B wins (winner_agent=5)
...
ALTER TABLE public.prompt_versions
  ...
  CHECK (agent_slot IN ('baseline','main','candidate_2','candidate_3','candidate_4'));
...
ALTER TABLE public.ai_judge_runs
  ...
  CHECK (winner_agent BETWEEN 1 AND 5);
...
-- ─── v10B prompt (candidate_4) ────────────────────────────────────────────────
-- v10 base with v11-era field names + bad-guessers ban + P4 direction fix + P5 note
...
VALUES (
  ...
  false,
  'candidate_4',
  $v10b_sys$You write a nightly WhatsApp roast for a private friends' World Cup prediction group.
...
BAD EXAMPLE
What a day in the World Cup. Bob had an amazing performance with 4 points. Alice is right behind and the race is heating up. Carol had a tough day with zero points, but will bounce back. The group got blindsided by the result. Tomorrow's danger: anyone, because anything can happen.$v10b_sys$,
  $v10b_usr$Here is today's data for {{group_name}}.
...
-- ─── v11-main-2 prompt ────────────────────────────────────────────────────────
-- v11 + P4 global_upset direction rule
...
VALUES (
  ...
  false,
  'main',
  $v11b_sys$You write a nightly WhatsApp roast for a private friends' World Cup prediction group.
...
BAD EXAMPLE
What a day in the World Cup. Bob had an amazing performance with 4 points. Alice is right behind and the race is heating up. Carol had a tough day with zero points, but will bounce back. The group got blindsided by the result. Tomorrow's danger: anyone, because anything can happen.$v11b_sys$,
  $v11b_usr$Here is today's data for {{group_name}}.
...
-- ─── v12-picks-2 prompt ───────────────────────────────────────────────────────
-- v12 + P4 direction fix + P5 scoreline note
...
VALUES (
  ...
  false,
  'candidate_2',
  $v12b_sys$PICKS RULE - READ THIS FIRST
...
- P4 must reference a specific number (percentage or count) from dist_group or dist_global$v12b_sys$,
  $v12b_usr$Here is today's data for {{group_name}}.
...
-- ─── v13-unique-2 prompt ──────────────────────────────────────────────────────
-- v13 + P4 direction fix + P5 scoreline note + P6 template phrase ban
...
VALUES (
  ...
  false,
  'candidate_3',
  $v13b_sys$UNIQUENESS RULE - READ THIS FIRST
...
- scan every paragraph opener: if it is a general observation, rewrite with a specific data fact$v13b_sys$,
  $v13b_usr$Here is today's data for {{group_name}}.
...
-- ─── fn_daily_admin_digest (add v10B wins) ────────────────────────────────────
