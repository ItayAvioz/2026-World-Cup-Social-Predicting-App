-- M86: Predictions count unique (user,game) pairs not raw rows (inflated by groups)
...
BEGIN
  ...
  -- Predictions: count unique (user_id, game_id) pairs to avoid group inflation
  -- new = first submitted today; edits = updated today but submitted before today
  SELECT
    COUNT(DISTINCT (user_id, game_id)) FILTER (WHERE submitted_at >= v_start AND submitted_at < v_end AND (is_auto = false OR is_auto IS NULL)),
    COUNT(DISTINCT (user_id, game_id)) FILTER (WHERE updated_at >= v_start AND updated_at < v_end AND submitted_at < v_start AND (is_auto = false OR is_auto IS NULL)),
