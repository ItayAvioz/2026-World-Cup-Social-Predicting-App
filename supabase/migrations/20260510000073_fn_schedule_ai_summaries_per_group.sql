-- Migration 73: fn_schedule_ai_summaries — schedule one cron job per qualifying group per date
-- Fixes timeout when >5 qualifying groups: each group gets its own EF invocation with { date, group_id }
      ...
      PERFORM cron.schedule(
        ...
        format(
          'SELECT net.http_post(
              ...
              body := jsonb_build_object(''date'', %L, ''group_id'', %L)::text
            )',
          v_ef_url,
          v_srk,
          v_day.game_date,
