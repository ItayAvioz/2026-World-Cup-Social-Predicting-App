-- M84: Add unique user counts for prediction_submit and pick_submit in daily digest
...
DECLARE
  ...
  v_pred_actions int; v_pred_users int;
  v_pick_actions int; v_pick_users int;
...
BEGIN
  ...
  SELECT
    ...
    COUNT(DISTINCT user_id) FILTER (WHERE event_type='prediction_submit'),
    COUNT(*) FILTER (WHERE event_type='pick_submit'),
    COUNT(DISTINCT user_id) FILTER (WHERE event_type='pick_submit'),
    COUNT(*) FILTER (WHERE event_type='share_click'),
    COUNT(*) FILTER (WHERE event_type='page_view')
  INTO v_pred_actions, v_pred_users, v_pick_actions, v_pick_users, v_share_clicks, v_page_views
  FROM public.app_events WHERE created_at >= v_start AND created_at < v_end;
  ...
  v_digest := jsonb_build_object(
    ...
    'prediction_users',   COALESCE(v_pred_users,0),
    'pick_actions',       COALESCE(v_pick_actions,0),
    'pick_users',         COALESCE(v_pick_users,0),
