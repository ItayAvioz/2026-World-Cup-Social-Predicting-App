-- Fix avg_session_seconds calculation in fn_daily_admin_digest
-- Bug: EXTRACT(EPOCH FROM MAX-MIN) measured wall-clock span between first/last heartbeat,
-- inflating time when tab was opened briefly across a long day.
-- Fix: COUNT(*) * 15.0 — each heartbeat = 15s of confirmed active/visible time.
