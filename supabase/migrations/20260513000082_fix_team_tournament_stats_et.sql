-- M82: fix team_tournament_stats view
-- W/D/L: use knockout_winner for ET/PEN games (was 90-min score only)
-- avg_goals: use COALESCE(et_score, score) for full-match goal count
...
SELECT
  ...
  count(*) FILTER (WHERE
    ...
    OR (g.score_home = g.score_away AND g.knockout_winner = ts.team)
  )                                                                           AS wins,
  count(*) FILTER (WHERE
    g.score_home = g.score_away AND g.knockout_winner IS NULL
  )                                                                           AS draws,
  count(*) FILTER (WHERE
    (g.team_home = ts.team AND g.score_home < g.score_away)
    OR (g.team_away = ts.team AND g.score_away < g.score_home)
    OR (g.score_home = g.score_away AND g.knockout_winner IS NOT NULL AND g.knockout_winner <> ts.team)
  )                                                                           AS losses,
  ...
  round(avg(
    CASE WHEN g.team_home = ts.team
      THEN COALESCE(g.et_score_home, g.score_home)
      ELSE COALESCE(g.et_score_away, g.score_away)
    END
  ), 1)                                                                       AS avg_goals_scored,
  round(avg(
    CASE WHEN g.team_home = ts.team
      THEN COALESCE(g.et_score_away, g.score_away)
