-- M76: Enhanced daily digest stats
-- Per-game: manual/auto split, auto_exact, auto_correct, wrong count
-- Share clicks tracking, peak hour in Israel time, v10B wins row
...
DECLARE
  ...
  v_pred_actions int; v_pick_actions int; v_page_views int; v_share_clicks int;
  v_judge_runs   int;
...
BEGIN
  ...
  -- Peak hour in Israel time (Asia/Jerusalem handles DST automatically)
  ...
  INTO v_pred_actions, v_pick_actions, v_share_clicks, v_page_views
  FROM public.app_events WHERE created_at >= v_start AND created_at < v_end;
