-- M111: align fn_daily_admin_digest with the 07:30-UTC match-day boundary (M110).
-- Two changes only:
--   1. Window starts at 07:30 UTC (was 00:00) so games + activity are the match-day, matching the summary grouping.
--   2. Summaries/tokens counted by the `date` field (the match-day) instead of `generated_at`. This fixes the
--      bug where summaries generated at 00:00 UTC (next calendar day) showed "0 created / 0 tokens" in the digest
--      while the game (counted by kick_off) showed up — they're now in the same report.
-- Run time (08:00 UTC) and v_yesterday computation unchanged: digest runs 30 min after the 07:30 close, sees a
-- complete match-day. The judge block already keyed on `date = v_yesterday`.
...
DECLARE
  ...
  v_start        timestamptz := (v_yesterday::text || ' 07:30:00+00')::timestamptz;
...
BEGIN
  ...
  -- Summaries/tokens by the match-day `date` field (not generated_at) so they align with the games above.
  SELECT COUNT(*), COALESCE(SUM(prompt_tokens),0), COALESCE(SUM(completion_tokens),0)
  INTO v_summ_created, v_tokens_in, v_tokens_out
  FROM public.ai_summaries WHERE date = v_yesterday;
