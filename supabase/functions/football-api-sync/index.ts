/**
 * football-api-sync v12 — better verify_player_ids (initial-detect + hyphen-tolerant lastname + nickname fallback)
 */
...
const TEAM_ALIASES: Record<string,string> = {"cote divoire":"ivory coast","korea republic":"south korea","cabo verde":"cape verde","usa":"united states","ir iran":"iran","turkiye":"turkey"}
...
async function handleProbeWcTeam(api_team_id: number): Promise<Response> {
  ...
  const raw = await footballApiPaginate(`/players?team=${api_team_id}&season=${WC_SEASON}`)
  const m: Map<number,any> = new Map()
  for (const r of raw) { const id = r.player?.id; if (!id) continue; const pos = r.statistics?.[0]?.games?.position ?? null; const ex = m.get(id); if (!ex) m.set(id, {api_player_id:id,name:r.player.name,position:pos,team:r.statistics?.[0]?.team?.name,appearences:r.statistics?.[0]?.games?.appearences}); else if (!ex.position && pos) ex.position = pos }
  return json({ status:'probe_wc_team_ok', api_team_id, total_players:m.size, players:Array.from(m.values()) })
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    switch (body.mode) {
      ...
      case 'probe_wc_team': return await handleProbeWcTeam(body.api_team_id)
