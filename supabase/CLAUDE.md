...
## Migrations (99 local + 3 MCP-only — all deployed)
...
| nightly-summary | **v33** (was v32) | ✅ ACTIVE | Single-group mode: accepts group_id in body, skips loop. Per-group cron architecture (M73). **v33 (2026-05-24, EF-audit fix):** hard group failure now logs `ef_errors('group_failed')` (was silent — cron discards response) → admin email + digest; judge-fail fallback now ships **agent 4 = baseline (v10-baseline)** instead of agent 1 (`main`). Skipped in test mode. Deployed via Mgmt API from local file. |
