...
| 127 (PROD ONLY) | **migrations-prod/**20260601000127_add_id_verification.sql | PROD-only: adds `id_verification text` column to `top_scorer_candidates`. Snapshot of 2026-06-01 reconciliation against api-football WC2026 squads (`/players/squads?team=X` for all 47 active teams via `football-api-sync` v10-v12 `verify_player_ids` + `bootstrap_squads` modes). Status values per row: `fully_verified` (991 — in api WC squad + name match), `squad_ok_name_suspect` (41 — in squad, name verifier false-positive on initial-format), `squad_ok_name_mismatch` (12 — in squad, api /profiles endpoint returned stale display name e.g. id 1697 Gavi → api says "Pablo Fornals", squad endpoint says Spain correct), `squad_missing_name_ok` (120 — api preliminary roster doesn't include them yet but name matches: Ronaldo, Memphis, Davies, Isak, Trossard, Kounde), `squad_missing_name_flagged` (17 — doubly suspect: mostly initial-format FPs + ~4 obscure Qatar/Iraq backup players). Total 1,181 active = sum. Mirror status saved per-player in `data/wc2026_squads.json` (1,181 unique ids match DB perfectly + 70 entries with null api_id + 18 duplicate-id pairs marked `no_api_id` / status carried from primary). Zero cross-team contamination across all 1,181. Confidence: ~99.7%. Schema change applied via `apply_migration`; per-row UPDATEs via `execute_sql`. |
