...
| 123 | 20260531000123_auto_predict_solo_users.sql | `fn_auto_predict_game`: add second loop covering solo users. Grouped loop (unchanged) iterates `group_members`; solo loop iterates `profiles` not in any group, inserts random 0–5 scoreline with `group_id = NULL` and `is_auto = true`. Before: M30 only covered grouped users (M36 explicitly noted ungrouped auto-predict as "future work") → solo users who forgot to submit had no row at all for the game. Brings predictions in line with `fn_auto_assign_picks` (M41) which has long had separate grouped + ungrouped branches for both champion and top scorer. ON CONFLICT (user_id, game_id, group_id) DO NOTHING leverages the UNIQUE NULLS NOT DISTINCT constraint from M36. Apply to BOTH envs. |
