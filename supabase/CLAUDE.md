...
## Migrations (103 local + 3 MCP-only — all deployed)
...
|---|---|---|---|
| football-api-sync | **v35** (was v34) | ✅ ACTIVE | Modes: probe, probe_date, probe_ns, verify, sync, sync_stats, sync_af_odds, setup, setup_lineups, snap_stats, probe_stats, probe_odds. **v35 (2026-05-24, EF-audit fixes #5/#3/#2):** local source now == deployed (probe_date/probe_ns folded into local — no more drift); `sync` terminal-status handler (PST/SUSP/ABD/CANC/AWD/WO/INT → unschedule + reportEfError, no infinite +5min loop) + KO+6h hard-stop safety net; `verify` >5min KO change also calls `fn_reschedule_game` (M107). Deployed via Mgmt API from local file → deployed == local by construction. |
| nightly-summary | **v34** (was v33) | ✅ ACTIVE | Single-group mode: accepts group_id in body, skips loop. Per-group cron architecture (M73). **v34 (2026-05-25):** day-window for "that day's games" shifted **00:00 → 07:30 UTC** (`dayStart/dayEnd`, lines ~576–579) to match the M110 match-day boundary so a US match-night isn't split. **v33 (2026-05-24, EF-audit fix):** hard group failure logs `ef_errors('group_failed')`; judge-fail fallback ships **agent 4 = baseline (v10-baseline)**. Skipped in test mode. Deployed via Mgmt API from local file. |
