...
const MAX_TOK        = 400
const JUDGE_MAX      = 350
// Prompts loaded by agent_slot (latest version_tag per slot, DESC order — same as EF)
const AGENT_SLOTS   = ['baseline', 'main', 'candidate_2', 'candidate_3', 'candidate_4']
const SLOT_PARAMS   = {
  'baseline':    { temperature: 0.6, seed: 42 },
  'main':        { temperature: 0.6, seed: 42 },
  'candidate_2': { temperature: 0.5, seed: 43 },
  'candidate_3': { temperature: 0.4, seed: 44 },
  'candidate_4': { temperature: 0.6, seed: 42 },
}
const PHASE_LABELS = {
...
const JUDGE_SYS = `You are a judge evaluating five nightly WhatsApp roast summaries for a World Cup prediction group.
Score each on 4 dimensions (0-10 each) and pick one winner.

SCORING WEIGHTS:
- accuracy (45%): No today_exact/total_exact confusion; streak = abs(streak); no invented facts; scorelines correct; champion result correct
- humor (30%): Picks as rivalry fuel when champion played; specific scoreline for worst performer; P4 unique angle with actual numbers (not template); personal not generic
...
- structure (10%): 6 paragraphs; P6 starts "Tomorrow's danger:"; exact point gap between rank 1 and rank 2 appears; streak referenced in P6
...
async function main() {
  ...
  // 1. Load prompts by agent_slot — latest version_tag per slot (same logic as EF)
  console.log('\nLoading prompts...')
  const allPrompts = await sbGet('prompt_versions?select=id,version_tag,agent_slot,system_prompt,user_prompt_template&agent_slot=not.is.null&order=version_tag.desc')
  const promptMap = {}  // slot → prompt row
  for (const p of allPrompts) {
    if (AGENT_SLOTS.includes(p.agent_slot) && !promptMap[p.agent_slot]) {
      promptMap[p.agent_slot] = p  // first = highest version_tag = latest
    }
  }
  for (const slot of AGENT_SLOTS) {
    if (!promptMap[slot]) { console.error(`No prompt found for slot: ${slot}`); process.exit(1) }
  }
  console.log(`Prompts loaded: ${AGENT_SLOTS.map(s => promptMap[s].version_tag).join(', ')}`)
  ...
  for (const date of DATES) {
    ...
    for (const group of qualifying) {
      ...
      for (let i = 0; i < AGENT_SLOTS.length; i++) {
        const slot = AGENT_SLOTS[i]
        const prm  = promptMap[slot]
        const par  = SLOT_PARAMS[slot]
        const tag  = prm.version_tag
