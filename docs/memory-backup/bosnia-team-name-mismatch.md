...
**🔁 RECURRENCE #2 — 2026-06-17 — DR Congo (3rd team, same Pattern B).** Portugal 1–1 DR Congo (game `ed00190a…`, fixture 1539003): DR Congo column "—". api stats spelling = **`Congo DR`** vs canonical **`DR Congo`** (word-order flip → `normalizeTeam` `congo dr` ≠ `dr congo`). Fix = football-api-sync **PROD v16**: added `"congo dr":"dr congo"` to `TEAM_ALIASES` (only line vs v15), smoke-tested OK. + PROD rename-backfill all 3 tables (26 player / 1 team / **1 event** — events present this time, renamed so scorer flag resolves). Post-fix full scan = empty (DR Congo was the last one). **This is now a recurring pattern (Bosnia → Cape Verde → DR Congo); api uses variant spellings esp. for African/playoff teams. Running `TEAM_ALIASES` stats-spelling entries: `cabo verde`, `cape verde islands`, `congo dr`.** When a new team first plays, eyeball its stats column — if blank, add the alias + rename. See [[stats-missing-diagnostic]].
